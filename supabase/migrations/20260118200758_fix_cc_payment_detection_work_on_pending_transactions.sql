/*
  # Fix Credit Card Payment Detection to Work on Pending Transactions

  ## Overview
  Updates the auto_detect_credit_card_payments function to match payments
  while they're still in pending status, so detection happens immediately
  after import instead of waiting for transactions to be posted.

  ## Changes
  - Modified function to match transactions with status = 'pending' instead of 'posted'
  - This allows auto-detection to work immediately after statement import
  - Keeps all other matching logic the same (amount, date, account type checks)
*/
  ...
  LOOP
    ...
    FOR v_cc_transaction IN
      ...
      ORDER BY
        ...
        ABS(EXTRACT(EPOCH FROM (t.date - v_bank_transaction.date)))
      LIMIT 1
    LOOP
