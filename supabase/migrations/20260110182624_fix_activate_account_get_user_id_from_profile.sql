/*
  # Fix activate_template_account - Get user_id from profiles table

  ## Problem
  The function tries to get user_id using auth.uid(), which returns NULL
  when called through RPC. This causes journal_entries INSERT to fail with:
  "null value in column user_id violates not-null constraint"

  ## Solution
  Get user_id from the profiles table using the profile_id parameter instead
  of relying on auth.uid()

  ## Impact
  - Account activation will work correctly through the frontend
...
DECLARE
  ...
  v_user_id uuid;
BEGIN
  -- FIXED: Get user_id from profiles table instead of auth.uid()
  SELECT user_id INTO v_user_id
  FROM profiles
  WHERE id = p_profile_id;
  
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'Profile % not found or has no user_id', p_profile_id;
  END IF;
...
'Activates a chart of accounts template for a profile. Fixed to get user_id from profiles table instead of auth.uid().';
