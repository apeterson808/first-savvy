/*
  # Fix apply_rule_to_transaction to Respect Status Gate

  1. Overview
    The apply_rule_to_transaction function was directly updating transaction status
    when auto_confirm_and_post was enabled, which violated the status gate protection.

  2. Changes
    - Update apply_rule_to_transaction to set session flag before status updates
    - Clear session flag after update
    - Ensure proper authorization for status changes
    - Fix column name from category_account_id to chart_account_id

  3. Security
    - Uses set_session_flag to authorize status changes
    - Maintains service boundary enforcement
    - Ensures all status changes go through proper channels
*/
