/*
  # Add account_name to Single Account Journal Lines Function

  1. Changes
    - Modify `get_account_journal_lines_paginated` to include `account_name` field
    - This ensures consistency with the multi-account version
    - The account_name will be the display name of the account being viewed

  2. Details
    - Adds account_name to the RETURNS TABLE definition
    - Joins with user_chart_of_accounts to get the account display name
    - Returns the account_name in the SELECT clause
*/

-- Drop and recreate the function with account_name field
...
v_account_name text;
BEGIN
-- Get account class and name
SELECT class, COALESCE(display_name, coa_template.display_name)
