/*
  # Simplify Audit Trail System - Use Same Reference Number

  ## Summary
  Simplifies the audit trail by allowing multiple journal entries to share
  the same reference number. This makes it clear which entries are related
  (post, undo, repost) while relying on created_at timestamp for ordering.

  ## Changes

  1. Drop Unique Constraint
     - Remove unique constraint on (profile_id, entry_number)
     - Allow multiple entries with same reference number
     - Rely on created_at timestamp for chronological ordering

  2. Update Undo Functions
     - Modify undo_post_transaction to reuse original entry_number
     - Modify undo_post_transfer_pair to reuse original entry_number
     - Modify undo_post_cc_payment_pair to reuse original entry_number
     - Remove calls to generate_journal_entry_number for reversals

  3. Update Posting Trigger
     - When re-posting a transaction, reuse the original entry_number
     - Maintains same reference across post/undo/repost lifecycle

  ## Business Logic
  - Post: Creates ADJ-0063 with entry_type = 'adjustment'
  - Undo: Creates ADJ-0063 with entry_type = 'reversal'
  - Repost: Creates ADJ-0063 with entry_type = 'adjustment'
  - All entries share same reference, distinguished by created_at and entry_type
...
-- 1. Drop unique constraint on entry_number
ALTER TABLE journal_entries
  DROP CONSTRAINT IF EXISTS unique_entry_number_per_profile;

-- 2. Update undo_post_transaction to reuse original entry_number
...
BEGIN
  ...
  -- Get the original entry_number to reuse
  SELECT entry_number INTO v_original_entry_number
  FROM journal_entries
  WHERE id = v_transaction.current_journal_entry_id;

  -- Create reversal journal entry with SAME entry_number
  INSERT INTO journal_entries (
    ...
    CURRENT_DATE,
    v_original_entry_number,  -- Reuse same reference number
  ...
  v_result := jsonb_build_object(
    ...
    'reversal_entry_number', v_original_entry_number
  );
...
-- 3. Update undo_post_transfer_pair to reuse original entry_number
...
BEGIN
  ...
  IF v_txn_count = 0 THEN
    RAISE EXCEPTION 'Transfer pair % not found', p_pair_id;
  END IF;

  IF v_txn_count != 2 THEN
    RAISE EXCEPTION 'Invalid transfer pair: expected 2 transactions, found %', v_txn_count;
  END IF;
  ...
  IF v_shared_je_id IS NULL THEN
    RAISE EXCEPTION 'Invalid state: no journal entry found for transfer pair';
  END IF;

  -- OPTIONAL SAFETY CHECK 6: Block reversing a reversal
  ...
  -- Create ONE reversal journal entry for the shared JE with SAME entry_number
  INSERT INTO journal_entries (
    profile_id,
    user_id,
    entry_date,
...
-- 4. Update undo_post_cc_payment_pair to reuse original entry_number
...
'Unposts a transaction by creating a reversal journal entry with the same reference number. Enforces profile membership and period locks.';

COMMENT ON FUNCTION undo_post_transfer_pair IS
'Unposts a transfer pair atomically with strict validation. Creates reversal entry with same reference number.';

COMMENT ON FUNCTION undo_post_cc_payment_pair IS
'Unposts a credit card payment pair atomically with strict validation. Creates reversal entry with same reference number.';
