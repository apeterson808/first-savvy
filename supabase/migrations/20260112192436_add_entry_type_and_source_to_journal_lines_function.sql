/*
  # Add Entry Type and Source to Journal Lines Function

  1. Changes
    - Add entry_type field to return type and query
    - Add source field to return type and query
    - These fields from journal_entries table will help UI identify and style opening balance entries

  2. Updated Return Columns
    - entry_type: Type of journal entry (opening_balance, adjustment, transfer, etc.)
    - source: Source of the entry (system, manual, import)
*/

-- Drop existing function to allow return type change
DROP FUNCTION IF EXISTS get_account_journal_lines_paginated(uuid, uuid, date, date, integer, integer);

-- Recreate function with entry_type and source fields
...
'Paginated version with running balance calculation. Returns journal lines for an account with pagination support, pre-calculated running balances, entry type, source, and total count. Only includes posted transactions and manual journal entries. Performance optimized for accounts with 50,000+ transactions.';