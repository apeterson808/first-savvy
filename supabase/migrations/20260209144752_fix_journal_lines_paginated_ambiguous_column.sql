/*
  # Fix ambiguous column reference in get_account_journal_lines_paginated

  ## Summary
  Fixes the "column reference 'line_id' is ambiguous" error that prevents
  the account register from displaying journal lines.

  ## Problem
  The RETURNS TABLE defines output columns with the same names as columns
  in the query, causing PL/pgSQL to be unable to determine if we're referring
  to the output variable or the query column.

  ## Solution
  Qualify all column references in the final SELECT to explicitly reference
  the CTE (lines_with_balance) instead of potentially conflicting with the
  RETURNS TABLE output columns.

  ## Impact
  - Fixes account register display
  - All journal lines will now show correctly in account detail pages
*/
