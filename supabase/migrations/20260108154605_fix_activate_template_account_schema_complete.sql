/*
  # Fix activate_template_account Function Schema Completely

  1. Problem
    - Function references columns that don't exist in user_chart_of_accounts table
    - user_id column was removed in migration 20260103221149
    - custom_display_name column was removed in migration 20251224235801
    - Only display_name column exists (not custom_display_name)

  2. Schema Mismatches Fixed
    - UPDATE statement: custom_display_name → display_name
    - INSERT columns: Remove user_id, remove custom_display_name
    - INSERT values: Remove v_user_id, map p_custom_display_name to display_name
    - SELECT statement: Remove COALESCE, use display_name directly

  3. Changes
    - Fix all 6 locations where non-existent columns are referenced
    - Maintain parameter name p_custom_display_name for API compatibility
    - Keep v_user_id variable logic for journal entry function calls
*/

-- Drop the broken function
DROP FUNCTION IF EXISTS activate_template_account(uuid, integer, text, numeric, text, text, date);

-- Recreate with correct schema matching user_chart_of_accounts table
...
'Activates a template account for a profile with optional opening balance. Opening balance is dated on the statement start date (not the day before) to ensure it appears as the first chronological entry in the account register. Schema corrected to match user_chart_of_accounts table structure.';