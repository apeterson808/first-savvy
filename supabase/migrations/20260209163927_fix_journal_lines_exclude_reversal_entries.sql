/*
  # Fix Account Register to Exclude Reversal Entries

  1. Changes
    - Update `get_account_journal_lines_paginated` function to exclude reversal entries
    - Add filter for `je.reverses_entry_id IS NULL` in both count and main queries
    - This ensures only "active" journal entries appear in the account register
    
  2. Rationale
    - Reversal entries (REV-xxxx) are internal accounting mechanics
    - Users should only see currently active, posted transactions in the register
    - Reversal entries will be available in the separate audit history view
*/
