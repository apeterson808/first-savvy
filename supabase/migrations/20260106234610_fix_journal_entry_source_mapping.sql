/*
  ...
  ## Problem
  Transactions can have source='api' but journal_entries only allow 'manual', 'import', 'system', 'migration'.
  This causes a constraint violation when creating journal entries from transactions.

  ## Solution
  Update the journal entry creation trigger to map transaction source values to valid journal entry sources:
  - 'api' → 'import' (programmatically created)
  - 'csv' → 'import'
  - 'ofx' → 'import'
  - 'manual' → 'manual'
  - Any other → 'import' (safe default)

  ## Changes
  - Recreate auto_create_journal_entry_from_transaction() function with proper source mapping
*/
...
COMMENT ON FUNCTION auto_create_journal_entry_from_transaction() IS 
  'Automatically creates journal entries when transactions are posted or categorized. Maps transaction sources to valid journal entry sources.';
