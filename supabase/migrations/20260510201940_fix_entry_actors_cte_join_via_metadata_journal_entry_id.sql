/*
  # Fix entry_actors CTE — Join via metadata journal_entry_id

  ## Problem
  The entry_actors CTE in both audit history functions joined audit_logs on
  entity_id = journal_entry_id. But post_transaction audit logs store entity_id
  as the TRANSACTION id (not journal entry id), with the journal entry id in
  metadata->>'journal_entry_id'. So the actor lookup was failing for all live-posted
  entries, causing "By" to show "—".

  ## Fix
  Change entry_actors to match on EITHER:
  - entity_id = journal_entry_id (old backfill-style entries where je.id was stored directly)
  - (metadata->>'journal_entry_id')::uuid = journal_entry_id (live-posted entries)
*/
...
BEGIN
  ...
  -- Match audit_logs to journal entries via EITHER entity_id OR metadata->>'journal_entry_id'
  entry_actors AS (
    SELECT DISTINCT ON (je_id)
      je_id,
      actor_display_name
    FROM (
      -- Live posts: entity_id = transaction_id, journal_entry_id in metadata
      SELECT
      ...
      UNION ALL
      -- Old style: entity_id = journal_entry_id directly
      SELECT
    ...
    ORDER BY je_id, created_at DESC
  ),
  base_lines AS (
    SELECT
