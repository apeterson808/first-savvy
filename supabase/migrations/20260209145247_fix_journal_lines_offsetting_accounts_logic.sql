/*
  # Fix offsetting accounts aggregation logic

  ## Summary
  Fixes the duplicate journal lines issue by correcting the offsetting accounts
  aggregation. The previous logic grouped by both journal_entry_id AND account_id,
  which was incorrect for the join logic.

  ## Problem
  The offsetting_accounts_agg was creating one row per account per journal entry,
  but the aggregation was including ALL accounts (including the current account)
  rather than just the offsetting accounts.

  ## Solution
  Change the offsetting_accounts_agg to only group by journal_entry_id and
  aggregate ALL accounts in that entry. Then in the display logic, we exclude
  the current account from the list.

  ## Impact
  - Each journal line appears exactly once
  - Correct display of offsetting accounts (shows the OTHER accounts, not self)
  - Fixes "Showing 26 of 13" display bug
*/
