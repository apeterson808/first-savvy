/*
  # Add Multi-Account Journal Lines Function

  1. New Function
    - `get_multi_account_journal_lines_paginated` - Gets journal lines for parent and child accounts
    - Accepts array of account IDs
    - Includes account name to distinguish between parent and child transactions
    - Returns same structure as existing function plus account information

  2. Purpose
    - Allow parent budget accounts to show transactions from both parent and all child accounts
...
BEGIN
  ...
  base_lines AS (
    SELECT
      ...
      CASE
        WHEN ucoa.class IN ('asset', 'expense') THEN
