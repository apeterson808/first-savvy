/*
  # Fix Credit Card Payment Detection - Use Type Field Instead of Amount Sign

  ## Overview
  The detection function was checking for negative amounts, but the data model stores
  all amounts as positive and uses the 'type' field to indicate direction.

  ## Changes
  - Change bank transaction filter from `amount < 0` to `type = 'expense'`
  - Keep credit card filter as `amount > 0` but also check `type = 'income'`
  - This aligns with how transactions are actually stored in the database
*/
