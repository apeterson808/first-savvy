/*
  # Fix Undo Row Amount Display

  ## Problem
  Undo rows in audit history show NULL for debit/credit amounts because the
  query hardcoded NULL::numeric for both. An undo reverses a posted transaction,
  so the reversal amount should be shown as a credit (positive) since it is
  removing a charge from the account.

  ## Fix
  Join transactions on entity_id to get the original transaction amount.
  For the undo row, set credit_amount = ABS(transaction.amount) so the
  frontend displays it as a positive green amount. Net change remains 0
  since the undo row is informational — the actual balance impact is reflected
  by the re-posted or pending transaction rows.
*/
...
BEGIN
  ...
  base_lines AS (
    ...
    -- Undo rows: show the original transaction amount as a credit (reversal)
    SELECT
      al.id                                                         AS bl_line_id,
      ...
      ABS(t_undo.amount)                                            AS bl_credit_amount,
