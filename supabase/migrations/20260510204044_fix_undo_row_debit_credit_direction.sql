/*
  # Fix Undo Row Debit/Credit Direction

  ## Problem
  Undo rows were returning ABS(transaction.amount) as credit_amount regardless
  of the original transaction direction. This made the amount show the wrong
  sign in the audit history table.

  ## Fix
  Join to the original journal entry line (via original_journal_entry_id) and
  SWAP debit/credit: what was debit becomes credit and vice versa. This correctly
  represents the reversal of the original entry in the same double-entry terms,
  so the frontend amount calculation automatically shows the right sign.
*/
...
BEGIN
  ...
  base_lines AS (
    ...
    -- Undo rows: flip the original journal entry line's debit/credit
    -- so the amount sign naturally reverses in the frontend calculation
    SELECT
      al.id                                                         AS bl_line_id,
      ...
      orig_jel.credit_amount                                        AS bl_debit_amount,
      orig_jel.debit_amount                                         AS bl_credit_amount,
    ...
    LEFT JOIN journal_entry_lines orig_jel
      ON orig_jel.journal_entry_id = t_undo.original_journal_entry_id
      AND orig_jel.account_id = p_account_id
    WHERE al.profile_id = p_profile_id
      AND al.action = 'undo_transaction'
      AND (p_start_date IS NULL OR al.created_at::date >= p_start_date)
      AND (p_end_date IS NULL OR al.created_at::date <= p_end_date)
...
BEGIN
  ...
  base_lines AS (
    SELECT
      ...
      CASE
        WHEN ucoa_current.class IN ('asset', 'expense')
          THEN COALESCE(jel.debit_amount, 0) - COALESCE(jel.credit_amount, 0)
        ELSE COALESCE(jel.credit_amount, 0) - COALESCE(jel.debit_amount, 0)
      END                                                           AS bl_net_change,
      jel.line_number                                               AS bl_line_number,
    ...
    LEFT JOIN user_chart_of_accounts ucoa3 ON ucoa3.id = t_undo.bank_account_id
    LEFT JOIN journal_entry_lines orig_jel
      ON orig_jel.journal_entry_id = t_undo.original_journal_entry_id
      AND orig_jel.account_id = t_undo.bank_account_id
    WHERE al.profile_id = p_profile_id
      AND al.action = 'undo_transaction'
      AND (p_start_date IS NULL OR al.created_at::date >= p_start_date)
      AND (p_end_date IS NULL OR al.created_at::date <= p_end_date)
