/*
  # Set created_by_user_id When Creating Journal Entries

  ## Problem
  The `create_journal_entry` function inserts journal_entries rows but never
  populates the `created_by_user_id` column that was added in a later migration.
  This means the "By" column in the Audit History tab always shows "—" because
  the audit history functions JOIN on `je.created_by_user_id` to get the actor name.

  ## Fix
  Update `create_journal_entry` to also set `created_by_user_id = p_user_id`
  on the journal_entries INSERT. No other logic changes.
*/
...
-- Insert journal entry (now includes created_by_user_id for actor attribution)
INSERT INTO journal_entries (
profile_id, user_id, created_by_user_id, entry_date, entry_number,
description, entry_type, source
) VALUES (
p_profile_id, p_user_id, p_user_id, p_entry_date, v_entry_number,
