/*
  # Show two rows per undo: original entry + reversal

  ## Problem
  When a transaction is undone, the journal entry is deleted. The audit history
  showed only a single "undo" row, losing the original categorization. This means
  the ledger appeared unbalanced — the original debit/credit disappeared with no
  corresponding entry to offset it.

  ## Fix
  Each undo_transaction audit log now produces TWO rows in the audit history:

  1. **Original row** — reconstructed from metadata.lines with the original
     entry_number, entry_type, description, and amounts. Net change = original
     direction (so it contributed to the running balance as it did when posted).

  2. **Reversal row** — the undo itself, with flipped debit/credit amounts and
     entry_type = 'undo'. Net change is the opposite, so the two rows cancel out
     and the running balance is correctly restored.

  The total_count is also updated to count 2 per undo row.

  ## Scope
  Only get_account_audit_history_paginated is updated here.
  get_multi_account_audit_history_paginated follows the same pattern and
  is updated in a separate migration if needed.
*/
...
BEGIN
  ...
  -- Each undo produces 2 rows (original + reversal), regular JE lines count as 1
  SELECT
    SUM(row_count) INTO v_total_count
  FROM (
  ...
  base_lines AS (
    -- Regular journal entry lines (live, not undone)
    SELECT
    ...
    -- Undo row A: reconstruct the ORIGINAL entry from metadata
    -- This re-adds the original amounts so the history shows what was there
    SELECT
      -- Use a deterministic sub-id for the original row (hash-based suffix)
      (al.id::text || '-orig')::uuid                                AS bl_line_id,
      ...
      -- Original debit for this account
      (
      ...
      -- Original credit for this account
      (
      ...
      -- Net change same direction as original (will be cancelled by row B)
      CASE
        WHEN v_account_class IN ('asset', 'expense')
        THEN COALESCE((
          SELECT (line_elem->>'debit_amount')::numeric
    ...
    -- Undo row B: the REVERSAL entry — flipped amounts, type = 'undo'
    -- Net change is opposite of row A, so the two cancel out
    SELECT
      al.id                                                         AS bl_line_id,
      al.id                                                         AS bl_entry_id,
      COALESCE(al.metadata->>'entry_number', 'UNDO')               AS bl_entry_number,
      ...
      -- Reversal: original credit becomes debit
      (
        SELECT (line_elem->>'credit_amount')::numeric
        FROM jsonb_array_elements(COALESCE(al.metadata->'lines', '[]'::jsonb)) AS line_elem
        WHERE (line_elem->>'account_id')::uuid = p_account_id
      ...
      -- Reversal: original debit becomes credit
      (
        SELECT (line_elem->>'debit_amount')::numeric
        FROM jsonb_array_elements(COALESCE(al.metadata->'lines', '[]'::jsonb)) AS line_elem
        WHERE (line_elem->>'account_id')::uuid = p_account_id
      ...
      'uncleared'::text                                             AS bl_cleared_status,
      al.created_at                                                 AS bl_created_at,
      -- Net change = negative of row A (cancels it out)
      CASE
        WHEN v_account_class IN ('asset', 'expense')
        THEN COALESCE((
          SELECT (line_elem->>'credit_amount')::numeric
