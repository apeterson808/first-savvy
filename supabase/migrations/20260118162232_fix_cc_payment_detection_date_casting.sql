/*
  # Fix Credit Card Payment Detection - Fix Date Casting in EXTRACT Function

  ## Overview
  Fixed date casting issues in the EXTRACT function when calculating date differences.

  ## Changes
  - Cast date subtraction properly for EXTRACT function
  - Use proper date interval calculation
*/
  ...
  LOOP
    ...
    FOR v_cc_transaction IN
      ...
      WHERE t.profile_id = p_profile_id
        ...
        -- Date proximity (within 3 days) - using simple date subtraction
        AND ABS(t.date - v_bank_transaction.date) <= 3
        AND (p_transaction_ids IS NULL OR t.id = ANY(p_transaction_ids))
      ...
      ORDER BY
        ...
        ABS(t.date - v_bank_transaction.date)  -- Closest date match
      LIMIT 1  -- Only match with best candidate
    LOOP
      -- Calculate confidence score
      v_amount_diff := ABS(v_cc_transaction.amount - v_bank_transaction.amount);
      v_date_diff := ABS(v_cc_transaction.date - v_bank_transaction.date);
