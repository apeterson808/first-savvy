/*
  # Fix audit history ordering to sort by action time (most recent first)

  Previously ordered by transaction_date then entry_number — which groups
  entries by their accounting date, not when the action actually occurred.
  Changed ORDER BY to bl_created_at DESC so the most recent action appears first.
*/
...
BEGIN
  ...
  ORDER BY lwb.bl_created_at DESC, lwb.bl_line_number DESC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$;
