/*
  # Fix Journal Trigger to Set current_journal_entry_id and original_journal_entry_id

  ## Problem
  The auto_create_journal_entry_from_transaction() function was setting journal_entry_id
  but not current_journal_entry_id and original_journal_entry_id. This causes the undo_post
  functions to fail because they require current_journal_entry_id.

  ## Solution
  Add the missing column assignments at the end of the function:
  - current_journal_entry_id: Always set to the newly created JE
  - original_journal_entry_id: Set only if NULL (preserves first JE)

  ## Impact
  - All newly posted transactions will have current_journal_entry_id set
  - Undo functionality will work properly
  - Maintains full audit trail of journal entries
*/
...
BEGIN
  ...
  ELSIF NEW.cc_payment_pair_id IS NOT NULL THEN
    v_counter_entry_type := 'adjustment';  -- Use adjustment counter for cc payments
    v_je_entry_type := 'adjustment';       -- Store as adjustment in journal_entries
  ELSE
    v_counter_entry_type := 'adjustment';  -- Use adjustment counter for regular transactions
    v_je_entry_type := 'adjustment';       -- Store as adjustment in journal_entries
  END IF;
...
'Creates journal entries when transactions are posted. Sets journal_entry_id, current_journal_entry_id, and original_journal_entry_id for full audit trail.';
