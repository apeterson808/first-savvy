/*
  # Fix Journal Entry Number Generation

  1. Changes
    - Fix entry number generation to only parse JE- prefixed entries
    - Prevents errors when other entry number formats exist (TEST-0001, etc)

  2. Bug Fix
    - Previous code failed when non-JE entries existed
    - Now filters to only JE- entries before parsing
*/
...
BEGIN
  ...
  -- Generate entry number - only look at JE- prefixed entries
