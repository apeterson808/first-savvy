/*
  # Fix Credit Card Payment Detection - Use Pending Status Instead of Posted

  ## Overview
  The detection function was checking posted transactions, but it should detect
  payments while they're still pending (before user reviews and posts them).

  ## Changes
  - Change both transaction filters from `status = 'posted'` to `status = 'pending'`
  - This allows credit card payments to be detected immediately when imported
  - User can review and accept/reject while transactions are still pending
  - When accepted, transactions can then be posted together
*/
...
BEGIN
  ...
  FOR v_bank_transaction IN
    ...
    WHERE t.profile_id = p_profile_id
      AND t.status = 'pending'  -- CHANGED: Look at pending transactions
      AND t.type = 'expense'  -- Expense from bank account (withdrawal)
  ...
  LOOP
    ...
    FOR v_cc_transaction IN
      ...
      WHERE t.profile_id = p_profile_id
        AND t.status = 'pending'  -- CHANGED: Look at pending transactions
        AND t.type = 'income'  -- Income to liability account (payment that reduces liability)
        AND t.cc_payment_pair_id IS NULL  -- Not already matched
        AND t.transfer_pair_id IS NULL  -- Not a transfer
      ...
      ORDER BY
        ...
        ABS(t.amount - v_bank_transaction.amount),  -- Closest amount match
    ...
    LOOP
      ...
      -- NOTE: Do NOT set type yet - that happens only when user accepts
      UPDATE transactions
      SET
