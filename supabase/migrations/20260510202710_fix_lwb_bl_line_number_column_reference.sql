/*
  # Fix lwb.bl_line_number Column Reference Error

  ## Problem
  The multi-account function's lines_with_balance CTE used `bl.*` in SELECT,
  which in PL/pgSQL doesn't always expose sub-columns for ORDER BY in the outer
  query. The single-account function referenced lwb.bl_line_number in ORDER BY
  but lines_with_balance only had it via the bl.* wildcard expansion.

  ## Fix
  Explicitly name all columns in lines_with_balance (no bl.*), so the outer
  SELECT and ORDER BY can reference them by name without ambiguity.
*/
