/*
  # Fix audit history action time to use posted_at instead of created_at

  ## Problem
  The "Action Time" column in audit history was showing je.created_at — the moment
  the draft JE was first created — not when the transaction was actually posted.
  Since JEs are now created as drafts on import, created_at is the import time,
  not the post time. Two events (original post and re-post after undo) showed
  identical timestamps because they shared the same JE.

  ## Fix
  Use COALESCE(je.posted_at, je.created_at) as the created_at for regular journal
  entry lines in get_account_audit_history_paginated. Undo rows already correctly
  use al.created_at from the audit_logs table.
*/
...
BEGIN
  ...
  base_lines AS (
    ...
    SELECT
      ...
      -- Use posted_at as the action time; fall back to created_at for manual JEs
