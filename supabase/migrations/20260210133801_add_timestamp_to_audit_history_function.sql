/*
  # Add Timestamp Column to Audit History Function

  ## Summary
  Adds created_at timestamp to the audit history function so users can track when actions
  (posting, undo, edits) were performed on journal entries.

  ## Changes
  1. Updates get_account_audit_history_paginated to include created_at timestamp
  2. Adds created_at to the function return type
  3. Passes created_at through all CTEs in the query

  ## Purpose
  - Enables audit trail tracking of when users performed actions
  - Shows action timestamp in the Audit History tab
  - Helps with internal tracking and compliance
*/

-- Drop and recreate function with created_at timestamp
...
'Returns paginated audit history with created_at timestamp for tracking when actions were performed.
Shows all journal entries including reversals with full metadata for internal audit tracking.';
