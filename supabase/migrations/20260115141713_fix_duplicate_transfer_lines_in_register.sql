/*
  # Fix Duplicate Transfer Lines in Account Register

  ## Problem
  When viewing transfers in the account register, each journal entry line appears
  twice because both transactions (source and destination) are linked to the same
  journal entry. The LEFT JOIN to transactions causes each line to multiply.

  ## Solution
  Use DISTINCT ON (jel.id) to ensure each journal entry line appears only once,
  taking the first matching transaction if multiple exist.

  ## Impact
  - Account registers will show each transfer once instead of twice
  - Running balances will be correct
  - Transaction status/cleared_status will use the first matching transaction
*/
...
BEGIN
  ...
  RETURN QUERY
  WITH base_data AS (
    -- Get all the data we need in one pass, ensuring each line appears only once
    SELECT DISTINCT ON (jel.id)
      jel.id,
    ...
    AND (p_start_date IS NULL OR je.entry_date >= p_start_date)
    AND (p_end_date IS NULL OR je.entry_date <= p_end_date)
    ORDER BY jel.id, t.id  -- Use first transaction if multiple exist
  ),
  ...
  lines_with_balance AS (
    SELECT
      ...
      -- Running balance (always calculate from oldest to newest)
  ...
  ORDER BY lwb.entry_date DESC, lwb.journal_number DESC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$;
...
'Optimized paginated version with running balance. Shows newest entries first (DESC order).
Uses DISTINCT ON to prevent duplicate lines when multiple transactions link to the same journal entry (transfers).';
