/*
  # Clean Up Duplicate Opening Balance Entries

  ## Problem
  Due to the bug in create_opening_balance_journal_entry, many opening balance
  journal entries have BOTH lines pointing to the same account (the checking/asset account)
  instead of one line for the account and one for Opening Balance Equity.

  ## What This Does
  1. Find all opening balance journal entries
  2. For each entry, check if both lines have the same account_id
  3. If so, update the second line to point to the Opening Balance Equity account (3000)
  4. Recalculate balances for affected accounts

  ## Impact
  - Fixes existing incorrect journal entries
  - Corrects account balances
...
BEGIN
  ...
  LOOP
    ...
    IF v_line1.account_id = v_line2.account_id THEN
      ...
      -- Get the Opening Balance Equity account for this user
      ...
      ELSE
        RAISE WARNING '  ✗ Cannot fix: Opening Balance Equity account (3000) not found for user %', 
          v_entry.user_id;
      END IF;
    END IF;
  END LOOP;
