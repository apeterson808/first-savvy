/*
  ...
  2. Security Improvements
    - Re-fix security definer views (they were recreated with SECURITY DEFINER)
    - Re-fix function search paths (they still show as mutable)
*/
...
CREATE INDEX IF NOT EXISTS idx_ai_contact_suggestions_profile_id 
ON ai_contact_suggestions(profile_id);
...
CREATE INDEX IF NOT EXISTS idx_credit_card_payment_patterns_bank_account_id 
ON credit_card_payment_patterns(bank_account_id);

CREATE INDEX IF NOT EXISTS idx_credit_card_payment_patterns_credit_card_account_id 
...
CREATE INDEX IF NOT EXISTS idx_transaction_categorization_memory_bank_account_id 
ON transaction_categorization_memory(bank_account_id);

CREATE INDEX IF NOT EXISTS idx_transaction_categorization_memory_category_account_id 
...
CREATE INDEX IF NOT EXISTS idx_transaction_rules_match_bank_account_id 
...
DROP INDEX IF EXISTS idx_ai_category_suggestions_suggested_category_account_id;
DROP INDEX IF EXISTS idx_ai_contact_suggestions_suggested_contact_id;

-- ============================================================================
-- 3. FIX SECURITY DEFINER VIEWS (RE-FIX)
-- ============================================================================

-- The views need to be recreated with explicit SECURITY INVOKER
DROP VIEW IF EXISTS v_profile_tabs_display CASCADE;
CREATE VIEW v_profile_tabs_display 
...
CREATE VIEW account_activity_summary
WITH (security_invoker = true)
AS
SELECT 
  a.id AS account_id,
...
-- ============================================================================
-- 4. RE-FIX FUNCTION SEARCH PATHS
-- ============================================================================

-- The functions need to be recreated with explicit search_path
-- This ensures they use the correct schema and prevents injection attacks

CREATE OR REPLACE FUNCTION calculate_total_child_budget_allocations(parent_account_id uuid)
RETURNS numeric
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION get_parent_budget(child_account_id uuid)
