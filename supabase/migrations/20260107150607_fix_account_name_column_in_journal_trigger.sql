/*
  # Fix Account Name Column Reference in Journal Entry Trigger

  ## Problem
  The auto_create_journal_entry_from_transaction() function references a non-existent
  column 'account_name' when looking up the Uncategorized account.

  ## Root Cause
  Line 33 of the function queries:
  WHERE account_name = 'Uncategorized'

  But the user_chart_of_accounts table only has 'display_name' column
  (renamed from custom_display_name in migration 20251224235801).

  ## Impact
  - Transactions cannot be inserted
  - Transaction import fails with "column account_name does not exist" error
  - Journal entries cannot be auto-created

  ## Fix
  Replace 'account_name' with 'display_name' in the WHERE clause.
*/
...
BEGIN
  ...
  IF NEW.category_account_id IS NULL THEN
    ...
    WHERE profile_id = NEW.profile_id
      AND display_name = 'Uncategorized'
      AND class = 'expense'
    LIMIT 1;
...
'Automatically creates journal entries for transactions. Fixed to use display_name instead of account_name when looking up Uncategorized account.';