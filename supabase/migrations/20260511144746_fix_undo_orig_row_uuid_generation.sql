/*
  # Fix undo original row UUID generation

  The previous migration used `(al.id::text || '-orig')::uuid` which is invalid
  since appending text makes it a non-UUID string. Replace with a proper UUID
  derived from the audit log id using md5.
*/
...
BEGIN
  ...
  base_lines AS (
    ...
    SELECT
      -- Derive a stable UUID for the original row from the audit log id
      ...
      COALESCE((al.metadata->>'transaction_date')::date, al.created_at::date) AS bl_entry_date,
      COALESCE((al.metadata->>'transaction_date')::date, al.created_at::date) AS bl_transaction_date,
    ...
    -- Undo row B: the REVERSAL entry with flipped amounts, type = 'undo'
    SELECT
      al.id                                                         AS bl_line_id,
      al.id                                                         AS bl_entry_id,
      COALESCE(al.metadata->>'entry_number', 'UNDO')               AS bl_entry_number,
