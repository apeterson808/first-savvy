/*
  # Filter draft JEs from audit history functions

  ## Problem
  The Audit History (account register) was showing rows for draft journal entries
  (pending transactions). Draft JEs represent work-in-progress — they should have
  no effect on balances, reports, or the audit trail until posted.

  ## Fix
  Add `AND je.status IN ('posted', 'locked')` to the base_lines CTE WHERE clause
  in both audit history functions. The total_count SELECT is updated to match.

  Also excludes 'voided' JEs (voided transactions / paired secondary JEs).

  ## Affected Functions
  - get_account_audit_history_paginated
  - get_multi_account_audit_history_paginated
*/

-- Single-account audit history
...
BEGIN
  ...
  base_lines AS (
    ...
    -- Undo row B: the REVERSAL entry — flipped amounts
    SELECT
      al.id                                                         AS bl_line_id,
      al.id                                                         AS bl_entry_id,
      COALESCE(al.metadata->>'entry_number', 'UNDO')               AS bl_entry_number,
