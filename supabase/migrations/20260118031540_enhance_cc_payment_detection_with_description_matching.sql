/*
  # Enhance Credit Card Payment Detection with Description Matching

  ## Overview
  Updates the credit card payment detection to use bank memo/description data
  to identify credit card payments more accurately.

  ## Changes
  1. Adds description-based keyword matching
  2. Boosts confidence when payment keywords are found
  3. Improves matching accuracy by analyzing transaction descriptions

  ## Keywords Detected
  - "CREDIT CARD", "CC PAYMENT", "CITI CARD", "AMEX", "DISCOVER", "CHASE"
  - "VISA", "MASTERCARD", "CARD PAYMENT", "EPAYMENT", "ONLINE PAYMENT"
*/

-- ============================================================================
-- Enhanced Auto-Detection Function with Description Matching
-- ============================================================================
  ...
  LOOP
    ...
    FOR v_cc_transaction IN
      ...
      WHERE t.profile_id = p_profile_id
        ...
        AND a.type = 'credit_card'  -- Specifically credit card
        ...
        AND (
          ...
          OR v_desc_lower LIKE '%' || LOWER(SPLIT_PART(COALESCE(a.account_name, ''), ' ', 1)) || '%'  -- First word of account name
        )
      ORDER BY
        ...
        CASE WHEN v_has_cc_keyword AND (
          ...
          v_desc_lower LIKE '%' || LOWER(SPLIT_PART(COALESCE(a.account_name, ''), ' ', 1)) || '%'
        ) THEN 0 ELSE 1 END,
...
Enhanced with description analysis to identify credit card payment keywords and institution names.
Matches based on amount (within 1% tolerance), date proximity (within 3 days), and description keywords.';
