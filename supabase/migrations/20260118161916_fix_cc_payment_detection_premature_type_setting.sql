/*
  # Fix Credit Card Payment Detection - Remove Premature Type Setting

  ## Overview
  The auto_detect_credit_card_payments function was incorrectly setting the transaction
  type to 'credit_card_payment' during auto-detection. This should only happen when
  the user explicitly accepts the match.

  ## Changes
  - Remove type field update from auto-detection logic
  - Keep all other matching metadata (pair_id, confidence, auto_detected, reviewed, pattern_id)
  - Type will be set only when user calls acceptPayment API
*/
