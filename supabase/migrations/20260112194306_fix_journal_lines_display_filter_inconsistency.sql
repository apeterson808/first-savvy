/*
  # Fix Journal Lines Display Filter Inconsistency

  1. Problem
    - `recalculate_account_balance` includes ALL journal entry lines
    - `get_account_journal_lines_paginated` filters by transaction status `(t.id IS NULL OR t.status = 'posted')`
    - This causes opening balance entries to be counted in balance but not displayed if linked to pending transactions
    - Opening Balance Equity shows correct balance ($7,959.22) but no transactions in register

  2. Root Cause
    - Opening balance journal entries should NEVER be linked to transactions
    - But if they are, they get filtered out from display while still being counted in balance
    - The filter `(t.id IS NULL OR t.status = 'posted')` is too restrictive for non-transaction entries

  3. Solution
    - Remove transaction status filter from `get_account_journal_lines_paginated`
    - Journal entries are the source of truth - all should be displayed
    - Transaction status is handled by the transaction table separately

  4. Impact
    - Opening Balance Equity register will now show all journal entry lines
    - Period Summary will show correct Total Debits and Total Credits
    - All accounts will show complete journal activity
*/
...
'Paginated version with running balance calculation. Returns ALL journal lines for an account (no transaction status filtering). Journal entries are the source of truth. Fixed to match recalculate_account_balance filtering logic.';
