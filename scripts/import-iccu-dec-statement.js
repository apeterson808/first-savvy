...
import { findOrCreateOpeningBalanceEquityAccount, findBankAccountByLastFour, hasExistingTransactions, createOpeningBalanceTransaction } from '../src/api/statementImport.js';
...
async function importIccuDecStatement() {
  ...
  for (const accountData of ICCU_DEC_DATA.accounts) {
    ...
    try {
      const bankAccount = await findBankAccountByLastFour(
        profileId,
      ...
      console.log(`Found bank account: ${bankAccount.display_name || bankAccount.name}`);
      ...
      if (!hasExisting && accountData.beginningBalance > 0) {
        ...
        try {
          ...
          console.log(`Using equity account: ${equityAccount.display_name}`);
      ...
      for (const txn of accountData.transactions) {
        try {
          ...
          errors.push(`${txn.description}: ${err.message}`);
        }
      }
